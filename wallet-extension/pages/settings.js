...
export default function Settings({ router }) {
  ...
  useEffect(() => {
    chrome.storage.local.get(['publicWalletInfo', 'encryptedWallet'], (result) => {
      ...
      try {
        const decryptedWallet = decryptWalletData(result.encryptedWallet);
        const parsedPublicInfo = JSON.parse(result.publicWalletInfo);
